/*
------ MAIN FILTER CLASS ------
This class is used to execute the logic involved in running parallelized filter using 
...
{    
    ...
    */
    initialize_instance_vars(array, input_function, block_size)
    {
        ...
        // Create the shared memorfy buffer and array wrapper for the filter array
        const filter_array_buffer = new SharedArrayBuffer(Int32Array.BYTES_PER_ELEMENT * array.length);
        const filter_array = new Int32Array(filter_array_buffer);
        this.filter_array_buffer = filter_array_buffer;
        ...
        return [input_array, filter_array];
    }
    ...
    */
        ...
        const worker_promises = this.worker_pool.map((worker, index) => {
            return new Promise((resolve, reject) => {
                ...
                const data = {
                    ...
                    filter_array_buffer: this.filter_array_buffer,
    ...
    /*
    * run_parallel_filter() -> This function executes the parallel filtration on the specified array using the specified function
    * 
    * INPUTS 
    ...
    */
    async run_parallel_filter({ array, input_function, block_size })
    {
        // Initialize the necessary object instance variables
        const [input_array, filter_array] = this.initialize_instance_vars(array, input_function, block_size);
        ...
        // Build the final filtered array using the filter array and input array
        let final_array = [];
...
*/
run_serial_filter({ array, input_function })
{
    // Get the current time
    const start = performance.now();
    ...
    for (let j = 0; j < array.length; j++) {
        if (input_function(array[j]))
        {
            final_array.push(array[j]);
        }
    }
