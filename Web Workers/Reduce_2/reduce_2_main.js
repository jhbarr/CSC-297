/*
------ REDUCE MAIN ------
This logic controls the creation of web workers that execute reduction on an array of specified size
-------------------------
*/
...
function create_workers(worker, data, indexChunks) {
    ...
    return new Promise((resolve, reject) => {
        worker.onmessage = (event) => {
            // Get the running partial sum from the worker
...
function initialize_workers(n_workers, max_chunk, arr_len)
{
    ...
    // TODO - do this outside of the timed region
    ...
    return [workerPromises, sharedArray]
}
...
export async function run_parallel_reduce(arr_len, n_workers, max_chunk)
{
    ...
    const [workerPromises, sharedArray] = initialize_workers(n_workers, max_chunk, arr_len); 

    // Attempt to run the workers and display the final filtered array
    let finalSum;
    try {
        finalSum = await run_workers(workerPromises, sharedArray); // wait for promise to resolve
    } catch (err) {
